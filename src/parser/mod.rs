...
impl<'a> Parser<'a> {
    ...
    pub fn parse_alter(&mut self) -> Result<Statement, ParserError> {
        ...
        match object_type {
            ...
            // unreachable because expect_one_of_keywords used above
            unexpected_keyword => Err(ParserError::ParserError(
                format!("Internal parser error: expected any of {{VIEW, TYPE, COLLATION, TABLE, INDEX, FUNCTION, AGGREGATE, ROLE, POLICY, CONNECTOR, ICEBERG, SCHEMA, USER, OPERATOR, SUBSCRIPTION}}, got {unexpected_keyword:?}"),
            )),
        }
    }
