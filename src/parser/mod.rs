...
impl<'a> Parser<'a> {
    ...
    fn parse_delete_setexpr_boxed(&self) -> Result<Box<SetExpr>, ParserError> {
    ...
    pub fn parse_delete(&self) -> Result<Statement, ParserError> {
    ...
    pub fn parse_replace(&self) -> Result<Statement, ParserError> {
        if !dialect_of!(self is MySqlDialect | GenericDialect) {
            return parser_err!(
    ...
    ///
    /// This is used to reduce the size of the stack frames in debug builds
    fn parse_insert_setexpr_boxed(&self) -> Result<Box<SetExpr>, ParserError> {
        Ok(Box::new(SetExpr::Insert(self.parse_insert()?)))
    }

    /// Parse an INSERT statement
    pub fn parse_insert(&self) -> Result<Statement, ParserError> {
    ...
    ///
    /// This is used to reduce the size of the stack frames in debug builds
    fn parse_update_setexpr_boxed(&self) -> Result<Box<SetExpr>, ParserError> {
        Ok(Box::new(SetExpr::Update(self.parse_update()?)))
    }

    pub fn parse_update(&self) -> Result<Statement, ParserError> {
        let or = self.parse_conflict_clause();
    ...
    pub fn parse_values(&self, allow_empty: bool) -> Result<Values, ParserError> {
