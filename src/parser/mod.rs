...
impl<'a> Parser<'a> {
    ...
    pub fn parse_drop(&mut self) -> Result<Statement, ParserError> {
        ...
        let object_type = if self.parse_keyword(Keyword::TABLE) {
            ...
            return self.expected_ref(
                "COLLATION, CONNECTOR, DATABASE, EXTENSION, FUNCTION, INDEX, OPERATOR, POLICY, PROCEDURE, ROLE, SCHEMA, SECRET, SEQUENCE, STATISTICS, STAGE, TABLE, TRIGGER, TYPE, VIEW, MATERIALIZED VIEW or USER after DROP",
                self.peek_token_ref(),
            );
        };
    ...
    pub fn parse_alter(&mut self) -> Result<Statement, ParserError> {
        ...
        match object_type {
            ...
            // unreachable because expect_one_of_keywords used above
            unexpected_keyword => Err(ParserError::ParserError(
                format!("Internal parser error: expected any of {{TEXT SEARCH, VIEW, TYPE, COLLATION, TABLE, INDEX, FUNCTION, AGGREGATE, ROLE, POLICY, CONNECTOR, ICEBERG, SCHEMA, USER, OPERATOR, STATISTICS}}, got {unexpected_keyword:?}"),
            )),
        }
    }
