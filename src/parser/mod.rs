...
impl<'a> Parser<'a> {
    ...
    /// Parse a SQL `TRUNCATE` statement
    ...
    /// Parse a SQL `ROLLBACK` statement
    ...
    /// Parse a SQL `CASE` expression e.g. `CASE WHEN text IS NULL THEN 0 ELSE 1 END`
    ...
    /// Parse a SQL `EXTRACT` expression e.g. `EXTRACT(YEAR FROM ...)`
    ...
    /// Parse a SQL `SUBSTRING` expression e.g. `SUBSTRING('text', 1, 2)`
    ...
    /// Parse a SQL `TRIM` expression e.g. `TRIM('text')`
