...
fn parse_create_view() {
    clickhouse().verified_stmt(r#"CREATE MATERIALIZED VIEW foo (`baz` String) AS SELECT bar AS baz FROM in"#);
    clickhouse().verified_stmt(r#"CREATE MATERIALIZED VIEW foo TO out (`baz` String) AS SELECT bar AS baz FROM in"#);
...
fn parse_array_join() {
    clickhouse().verified_stmt(
        r#"SELECT asset AS a FROM runs AS r ARRAY JOIN r.assets AS asset"#,
    );
    clickhouse().verified_stmt(
        r#"SELECT asset AS a FROM runs ARRAY JOIN r.assets AS asset"#,
    );
}
