...
fn parse_with_clause_common_scalar_expression() {
    let dialects = all_dialects_where(|d| d.supports_common_scalar_expressions());

    // Plain literal scalar.
    let query = dialects.verified_query("WITH 42 AS answer SELECT answer FROM t");
    ...
    dialects.verified_stmt("WITH user_id AS uid SELECT uid FROM t");

    // Mixing a common scalar expression with a real CTE in the same WITH list.
