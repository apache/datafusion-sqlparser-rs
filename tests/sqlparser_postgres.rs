...
fn parse_bad_if_not_exists() {
    ...
    assert_eq!(
        ParserError::ParserError("Expected end of statement, found: EXISTS\nNear `CREATE TABLE NOT`".to_string()),
        res.unwrap_err()
    );
    ...
    assert_eq!(
        ParserError::ParserError("Expected end of statement, found: uk_cities\nNear `CREATE TABLE IF`".to_string()),
        res.unwrap_err()
    );
            ...
            source: CopySource::Query(Box::new(Query {
                ...
                body: Box::new(SetExpr::Select(Box::new(Select {
                    ...
                    projection: vec![
                        ...
                        SelectItem::ExprWithAlias {
                            expr: Expr::Value(Value::SingleQuotedString("hello".into())).empty_span(),
                            alias: Ident {
                                value: "b".into(),
                                quote_style: None,
                            }.empty_span(),
        ...
        Statement::Update { returning, .. } => {
            assert_eq!(
                Some(vec![
                    ...
                    SelectItem::UnnamedExpr(Expr::Identifier(Ident::new("prcp").empty_span()).empty_span()).empty_span(),
                ]),
                returning
            );
        }
