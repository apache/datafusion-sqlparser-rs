...
fn parse_explain_with_option_list() {
        ...
        Some(UtilityOptionList {
            options: vec![
                UtilityOption {
                    ...
                    arg: Some("TEXT".to_string()),
                },
                UtilityOption {
                    name: Ident::new("FORMAT2"),
                    arg: Some("JSON".to_string()),
                },
                UtilityOption {
                    name: Ident::new("FORMAT3"),
                    arg: Some("XML".to_string()),
                },
                UtilityOption {
                    name: Ident::new("FORMAT4"),
                    arg: Some("YAML".to_string()),
                },
            ],
        }),
    );
    ...
    run_explain_analyze_with_postgres_dialect(
        ...
        Some(r#"EXPLAIN ( NUM1 10, NUM2 10.1, NUM3 -10.2 ) SELECT sqrt(id) FROM foo"#),
        false,
        false,
        None,
        Some(UtilityOptionList {
            options: vec![
                ...
                UtilityOption {
                    name: Ident::new("NUM2"),
                    arg: Some("10.1".to_string()),
                },
                UtilityOption {
                    name: Ident::new("NUM3"),
                    arg: Some("-10.2".to_string()),
                },
            ],
        }),
    );
...
fn run_explain_analyze_with_postgres_dialect(
    query: &str,
    expcted_canonical: Option<&str>,
    expected_verbose: bool,
    expected_analyze: bool,
    expected_format: Option<AnalyzeFormat>,
    expcted_options: Option<UtilityOptionList>,
) {
    let test_dialect = TestedDialects {
        dialects: vec![Box::new(PostgreSqlDialect {}) as Box<dyn Dialect>],
        options: None,
    };
    let stmt = if let Some(canonical) = expcted_canonical {
        test_dialect.verified_stmt_with_canonical(query, canonical)
    } else {
        test_dialect.verified_stmt(query)
    };
    ...
    match stmt {
        Statement::Explain {
            ...
            assert_eq!(options, expcted_options);
