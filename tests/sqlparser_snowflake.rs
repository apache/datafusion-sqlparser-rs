    ...
    match snowflake_without_unescape().verified_stmt(sql) {
        Statement::CreateStage { file_format, .. } => {
            assert!(file_format.options.contains(&KeyValueOption {
                ...
                option_value: KeyValueOptionKind::Single(Value::Placeholder("AUTO".to_string()).with_empty_span()),
            }));
            assert!(file_format.options.contains(&KeyValueOption {
                option_name: "BINARY_FORMAT".to_string(),
                option_value: KeyValueOptionKind::Single(Value::Placeholder("HEX".to_string()).with_empty_span()),
            }));
            assert!(file_format.options.contains(&KeyValueOption {
